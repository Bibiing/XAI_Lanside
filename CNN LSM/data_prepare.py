...
def get_feature_data(tif_paths, window_size, norm_param_path=None):
    """
    Read features and min-max normalization, optionally save normalization params
    """
    n = int(window_size/2)
    data = []
    norm_params = {}
    for tif_data in os.listdir(tif_paths):
        ...
        normed, min_val, max_val = normalize_min_max(img)
        norm_params[tif_data] = {'min': float(min_val), 'max': float(max_val)}
        w, h = img.shape[0], img.shape[1]
        print(str(tif_data)+' amann enjoy...')
        normed = np.pad(normed, ((n, n), (n, n)), 'constant', constant_values=(0, 0))  # Filling edges
        data.append(normed)
    if norm_param_path:
        with open(norm_param_path, 'w') as f:
...
def save_to_tif(data_path, pred_result):
    ...
    dataset = driver.Create('simulasi/lsm.tif', im_width, im_height, im_bands, datatype)
